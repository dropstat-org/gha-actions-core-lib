 ...
 */
export class TerragruntGitInfo {
  /**
   ...
   */
  static async getWorkingDir(moduleDir: string): Promise<string> {
    let stdout = '';
    ...
    return parsed.WorkingDir as string;
  }
   ...
   */
  static async get(moduleDir: string): Promise<ModuleGitInfo> {
    let workingDir: string;
    try {
      workingDir = await this.getWorkingDir(moduleDir);
    } catch {
      Logger.warn(`TerragruntGitInfo: could not run terragrunt-info in '${moduleDir}'`);
      return {
    ...
    try {
      ...
      const info: ModuleGitInfo = { workingDir, gitUrl, gitBranch, gitCommitHash };
      Logger.info(`Infra module git info: ${JSON.stringify(info)}`);
      return info;
    } catch {
      Logger.warn(`TerragruntGitInfo: could not read git info in '${workingDir}'`);
      return {
        workingDir,
        gitUrl: 'No disponible',
        gitBranch: 'No disponible',
  ...
  static validateBranchMatch(gitBranch: string, deployBranch: string): boolean {
    if (gitBranch === 'No disponible') return true; // already warned upstream
    ...
    if (hasSlash || (!matches && !isTag)) {
      Logger.warn(
        `Infra branch mismatch: module is on '${gitBranch}' but expected '${deployBranch}'. ` +
   ...
   */
  static async getModuleGroups(workingDir?: string): Promise<Record<string, string[][]>> {
    let stdout = '';
    ...
    try {
      ...
      Logger.warn('TerragruntGitInfo: could not parse terragrunt output-module-groups JSON');
      return {};
    }
  }
}