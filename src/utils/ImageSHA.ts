 ...
 */
export function resolveImageSHA(): string {
  if (process.env.IMAGE_SHA) return process.env.IMAGE_SHA;
...
function getSecondParent(sha: string): string | null {
  try {
    const output = execSync(`git log --pretty=%P -n 1 ${sha}`, {
      encoding: 'utf8',
