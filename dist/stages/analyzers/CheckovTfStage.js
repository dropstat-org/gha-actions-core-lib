 ...
 */
class CheckovTfStage extends AbstractAnalyzerStage_1.AbstractAnalyzerStage {
    resultMap() {
    ...
    resolveSoftFail(stage, platformSoftFail) {
        if (stage.checkov?.softFail !== undefined)
            return stage.checkov.softFail;
    ...
    buildArgs(stage, planFile) {
        ...
        if (externalDir)
            args.push('--external-checks-dir', externalDir);
        return args;
    }
    /**
     ...
     */
    aggregateResults(results, softFail) {
        const values = [...results.values()];
        if (!softFail && values.includes('failure'))
            return 'failure';
        if (values.includes('warning') || (softFail && values.includes('failure')))
            return 'warning';
        return 'success';
    }
    logPerAccountResults(results, stageName) {
        Logger_1.Logger.info(`CheckovTfStage '${stageName}' — per-account results:`);
        for (const [account, result] of results) {
            const icon = result === 'success' ? '✅' : result === 'warning' ? '⚠️' : '❌';
            Logger_1.Logger.info(`  ${icon} ${account}: ${result}`);
        }
    }
    /** Extracts account name from a per-account plan filename: tfplan{n}-{account}.json */
    resolveAccountName(filePath) {
    ...
    async run(stage) {
        ...
        try {
            const { soft_fail: platformSoftFail } = (await PlatformConfigLoader_1.PlatformConfigLoader.securityPolicy()).checkov;
            ...
            try {
                ...
                Logger_1.Logger.warn('CheckovTfStage: could not restore analysis-source artifact — ' +
            ...
            Logger_1.Logger.info(`CheckovTfStage: scanning ${planFiles.length} plan file(s)`);
            ...
            Logger_1.Logger.info(`Aggregate result: ${aggregate}`);
...
exports.CheckovTfStage = CheckovTfStage;
//# sourceMappingURL=CheckovTfStage.js.map