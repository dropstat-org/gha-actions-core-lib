...
function makeStageConfig(overrides) {
    return { name: 'checkov_tf', ...overrides };
}
function stage(overrides) {
    return new CheckovTfStage_1.CheckovTfStage(makeConfig(overrides));
}
const mockExec = exec.exec;
const mockFindFiles = StageTransfer_1.StageTransfer.findFiles;
const mockRestoreByName = StageTransfer_1.StageTransfer.restoreByName;
...
describe('CheckovTfStage — install', () => {
    it('installs checkov via pip before scanning', async () => {
        ...
        expect(mockExec).toHaveBeenNthCalledWith(1, 'pip', ['install', 'checkov==3.2.527', '--quiet']);
    });
});
// ── artifact restore ──────────────────────────────────────────────────────────
...
//# sourceMappingURL=CheckovTfStage.test.js.map