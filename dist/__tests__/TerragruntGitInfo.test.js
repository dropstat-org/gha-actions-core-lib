...
describe('TerragruntGitInfo.validateBranchMatch', () => {
    ...
    it('returns false for a branch with slash even when it contains deployBranch', () => {
        ...
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('feature/develop-sync', 'develop')).toBe(false);
    });
    it('returns true when on a semver tag regardless of deployBranch', () => {
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('v1.2.3', 'master')).toBe(true);
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('1.0.0', 'develop')).toBe(true);
    });
    it('returns false when gitBranch does not contain deployBranch and is not a tag', () => {
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('master', 'develop')).toBe(false);
    });
    it('returns false when gitBranch contains a slash', () => {
        // slash → detached or remote ref — always mismatch per Groovy rule
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('origin/develop', 'develop')).toBe(false);
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('refs/heads/master', 'master')).toBe(false);
    });
    it('returns true when gitBranch is "No disponible" (upstream already warned)', () => {
        expect(TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('No disponible', 'develop')).toBe(true);
    });
    it('logs a warning on mismatch', () => {
        const core = require('@actions/core');
        TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('master', 'develop');
        expect(core.warning).toHaveBeenCalledWith(expect.stringContaining('Infra branch mismatch'));
    });
    it('does not log a warning on match', () => {
        const core = require('@actions/core');
        TerragruntGitInfo_1.TerragruntGitInfo.validateBranchMatch('develop', 'develop');
...
describe('TerragruntGitInfo.getModuleGroups', () => {
    it('returns parsed JSON from stdout', async () => {
        ...
        mockExec.mockImplementationOnce((_bin, _args, opts) => {
            opts.listeners?.stdout?.(Buffer.from(JSON.stringify(payload)));
            return Promise.resolve(0);
        });
        const result = await TerragruntGitInfo_1.TerragruntGitInfo.getModuleGroups();
...
describe('TerragruntGitInfo.get', () => {
    it('returns fallback info when terragrunt-info fails', async () => {
        ...
        const info = await TerragruntGitInfo_1.TerragruntGitInfo.get('/nonexistent');
    ...
    it('populates all fields when commands succeed', async () => {
        ...
        mockExec.mockImplementationOnce((_b, _a, opts) => {
        ...
        const info = await TerragruntGitInfo_1.TerragruntGitInfo.get('/some/module');
...
//# sourceMappingURL=TerragruntGitInfo.test.js.map